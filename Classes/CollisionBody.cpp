...
CollisionCapsule worldSpaceCapsule(const cocos2d::Node* p, const CollisionCapsule &c)
{
	...
	newShape.sPosition = p->convertToWorldSpace(c.sPosition) / cocos2d::Director::getInstance()->getRunningScene()->getScale();
	newShape.ePosition = p->convertToWorldSpace(c.ePosition) / cocos2d::Director::getInstance()->getRunningScene()->getScale();
	newShape.radius = c.radius * p->getParent()->getScale();
